import { loadDurableState, queueDurableState, flushDurableState } from './modules/storage.mjs';

(() => {
  'use strict';
  // Clarity 5.1.1 Web — hr-HR live captions through the speech service exposed by the user's browser/platform.
  ...
  const defaultPreferences = {
    ...
    // AI dorada: lokalni Whisper model koji radi isključivo u pregledniku (WebAssembly/WebGPU).
    // Bez API ključa, bez servera, bez troška — model se preuzme jednom i ostaje predmemoriran.
    aiRefine: false,
    aiModel: 'auto',
  ...
  // Modeli su unaprijed pretvoreni u ONNX format za transformers.js i preuzimaju se
  // izravno s Hugging Face CDN-a pri prvom korištenju (besplatno, bez ključa).
  const AI_MODELS = {
    base: 'Xenova/whisper-base',
    small: 'Xenova/whisper-small',
    turbo: 'onnx-community/whisper-large-v3-turbo'
  };
  const AI_ENGINE_URL = 'https://cdn.jsdelivr.net/npm/@huggingface/transformers@3.8.1';
  ...
  const RECOGNITION_START_TIMEOUT_MS = 5500;
  ...
  function scheduleRecognitionStart(delay = RECOGNITION_REARM_DELAY_MS, message = '') {
    ...
    else updateStatus('listening', 'Hrvatski · speech servis uređaja');
  }
  ...
  async function startListening() {
    ...
    else if (IS_IOS_WEBKIT) dom.soundStateText.textContent = 'Mikrofon uređaja';
    ...
    if (IS_IOS_WEBKIT && lastManualStopAt) {
  ...
  function createAndStartRecognition() {
    ...
    instance.continuous = !IS_IOS_WEBKIT;
    ...
    instance.onstart = () => {
      ...
      updateStatus('listening', window.__clarityNativeSpeech ? 'Hrvatski · speech servis uređaja' : 'Hrvatski · speech servis uređaja');
      startElapsedTimer();
    };
  ...
  // ——— Opcionalna lokalna AI dorada (Whisper u pregledniku) ———
  //
  // Web Speech je jedini izvor novih redova prijepisa. Whisper je opcionalni,
  // konzervativni korektor koji smije mijenjati samo već postojeći browser segment
  // kada se oba kanala gotovo potpuno slažu.
